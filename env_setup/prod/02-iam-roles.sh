#!/bin/bash
# Create IAM roles for PRODUCTION (us-east-2).
# Isolated from dev/qa — sources env-config-prod.sh only.
#
# Roles:
#   pf-syn-schedulingagents-bot-task-role-prod       — ECS task role (app permissions)
#   pf-syn-schedulingagents-bot-execution-role-prod  — ECS execution role (ECR + logs)
#
# Usage:
#   bash env_setup/prod/02-iam-roles.sh
...
echo "═══════════════════════════════════════════════════════════"
echo "  PF Scheduling Bot — IAM Roles (PRODUCTION)"
